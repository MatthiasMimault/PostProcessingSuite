...
15 - Radial Velocity included
16 - Binary write
17 - ForceVisc, RadVelYZ
@author: MM42910
"""
...
for name in CsvList:
    ...
    fic.write(b'FIELD FieldData '
     +bytes(str(len(dP_sca)+len(dP_vec)+4), 'utf-8')+b'\n')
    ...
    for row in dForce:
        ...
        +st.pack(">f", float(row['Force.y']))
        +st.pack(">f", float(row['Force.z']))) 
    fic.write(b'\n')    
    ...
    fic.write(b'RadVel_YZ 1 '+bytes(str(N), 'utf-8')+b' float\n')
    for row in dP:
