...
#        data += "{} {} {}\n".format(row._1, row._2, row._3) 
    ...
    # Field variables    
    dataField = "FIELD FieldData {}\n".format(len(dP_sca)+len(dP_vec)+4)
    ...
    dataField += "{} {} {} {}\n".format("Force_Mg", 1, N, "float")
    for row in dForce:        
        dataField += "{} ".format(np.sqrt(
                row['Force.x']**2+row['Force.y']**2+row['Force.z']**2))
    dataField += "\n"
    
    # Inclusion Magnitude Strain
    dataField += "{} {} {} {}\n".format("Strain_Mg", 1, N, "float")
    for row in dP:       
        dataField += "{} ".format(np.sqrt(float(row['StrainDot.x'])**2
                      +float(row['StrainDot.y'])**2
                      +float(row['StrainDot.z'])**2))
    dataField += "\n"
    
    # Inclusion Radial velocity Y+Z
    dataField += "{} {} {} {}\n".format("RadVelYZ", 1, N, "float")
    for row in dP:       
        dataField += "{} ".format(np.sqrt(float(row['Vel.y'])**2
                      +float(row['Vel.z'])**2))
    dataField += "\n"
